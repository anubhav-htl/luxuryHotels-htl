...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const iframe_guide = document.getElementById('iframe_guide');

  iframe_guide.addEventListener('click', function () {
    modal.show();
  });
  //end google maps modal instructions


  //start dropzone
  const maxImages = window.maxImages;
  ...
  let myDropzone = new Dropzone(dropzoneElement, {
    ...
    maxFilesize: 2,
    dictDefaultMessage: `Drag an image here or click to select from your files. You currently have ${10 - maxImages}, and can add ${maxImages} more.`,
    maxFiles: maxImages,
    parallelUploads: maxImages,
    paramName: 'file',
    acceptedFiles: 'image/*',
    addRemoveLinks: true,
    headers: {
  ...
  Array.from(forms).forEach(form => {
    form.addEventListener('submit', function (event) {
      if (!form.checkValidity() || (maxImages === 10 && uploadedImageNames.length === 0)) {
